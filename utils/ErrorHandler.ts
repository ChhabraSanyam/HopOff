...
export class ErrorHandler {
  ...
  static processError(error: unknown): ErrorInfo {
    ...
    if (
      ...
      typeof error === "object" &&
      "code" in error &&
      error.constructor.name === "LocationManagerError"
    ) {
    ...
    switch (error.code) {
      case LocationError.PERMISSION_DENIED:
        return {
          ...
          userMessage:
            "Location permission is required to set destination alarms.",
          recoveryActions: [
      ...
      case LocationError.LOCATION_UNAVAILABLE:
        return {
          ...
          message: error.message,
          userMessage:
            "Location services are disabled. Please enable them to use HopOff.",
          recoveryActions: [
            this.getSettingsAction("location"),
            this.getRetryAction(),
          ],
      ...
      case LocationError.TIMEOUT:
        return {
          ...
          message: error.message,
          userMessage:
            "Unable to get your location. Please try again or move to an area with better GPS signal.",
          recoveryActions: [
            this.getRetryAction(),
      ...
      case LocationError.GEOFENCING_UNAVAILABLE:
        return {
          ...
          message: error.message,
          userMessage:
            "Background location monitoring is not available. The app will use battery-intensive location polling.",
          recoveryActions: [
      ...
      case LocationError.GEOFENCE_LIMIT_EXCEEDED:
        return {
          ...
          message: error.message,
          userMessage:
            "Too many active location monitors. Please cancel existing alarms first.",
          recoveryActions: [
  ...
  private static processGenericError(error: Error): ErrorInfo {
    ...
    if (message.includes("network") || message.includes("fetch")) {
      return {
        ...
        message: error.message,
        userMessage:
          "Network connection is unavailable. Some features may not work.",
        recoveryActions: [
  ...
  static getDisplayMessage(errorInfo: ErrorInfo): string {
    ...
    if (errorInfo.recoveryActions.length > 0) {
      const primaryAction = errorInfo.recoveryActions.sort(
