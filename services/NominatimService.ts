...
export class NominatimServiceImpl implements NominatimService {
  ...
  private readonly userAgent = 'HopOff/1.0.0 (React Native App)';
  private readonly requestTimeout = 10000; // 10 seconds
  private lastRequestTime = 0;
  private readonly minRequestInterval = 1000; // 1 second between requests (Nominatim rate limit)
  ...
  async searchAddress(query: string, limit: number = 5): Promise<AddressSearchResult[]> {
    try {
      ...
      const results = data.map((item, index) => this.convertNominatimResult(item, index));
   ...
   */
  async reverseGeocode(coordinate: Coordinate): Promise<AddressSearchResult | null> {
    try {
      // Validate coordinate
      ...
      return this.convertNominatimResult(data, 0);
    } catch (error) {
      if (error instanceof NominatimServiceError) {
        throw error;
      }
   ...
   */
  private convertNominatimResult(result: NominatimResult, index: number): AddressSearchResult {
